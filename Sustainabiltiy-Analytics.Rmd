...
https://www.next-kraftwerke.de/energie-blog/preisschwankungen-regelenergie

https://www.swissgrid.ch/content/dam/swissgrid/about-us/newsroom/dossiers/control-energy-market-en.pdf
...
The following graphs show the power generation of with solar and wind power in Switzerland using the available hourly data from the entsoe platform ([Data Source for generation share of wind and solar (hourly)]).
...
Time series after log-transformation as well as first-order and seasonal differencing still shows a few spikes. We will try to look for outliers.

```{r, outlier_detection, eval = F, echo = FALSE, warning = F}
# Install necessary package
install.packages("tsoutliers")
library(tsoutliers)

# Detect outliers in the time series
outliers <- tso(baseload_log_seasonal_diff_d)

# View detected outliers
print(outliers)

# Plot the outliers in the time series
plot(outliers)

# You can also replace the outliers by using the following function
baseload_clean <- tsclean(baseload_log_seasonal_diff_d)
plot(baseload_clean, main="Outlier Corrected Time Series")
...
arima_model_baseload <- auto.arima(baseload_ts, seasonal = FALSE)

# Summary of the fitted SARIMA model
summary(arima_model_baseload)

# Forecast the next period
arima_forecast_baseload <- forecast(arima_model_baseload, h = 100)
